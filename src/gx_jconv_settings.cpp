 ...
 * --------------------------------------------------------------------------
 *
 *  This is the JConv settings handler class used by Jc_Gui
...
namespace gx_jconv
  {
    ...
    GtkWidget * gx_knob(const char* label,int mode, float init, float min, float max, float step)
    {
      ...
      //g_signal_connect(box, "expose-event", G_CALLBACK(gx_gui::box10_expose), NULL);
    ...
    void gx_setting_jconv_dialog_gui(GtkWidget *widget, gpointer data)
    {
      ...
      gtk_widget_modify_fg (gx_gui::label1, GTK_STATE_NORMAL, &colorGreen);
      ...
      GtkWidget *button_text = gtk_bin_get_child(GTK_BIN(ok_button));

      gdk_color_parse("#a6a9aa", &colorGreen);
      gtk_widget_modify_fg (button_text, GTK_STATE_NORMAL, &colorGreen);
      gdk_color_parse("#000000", &colorGreen);
      gtk_widget_modify_bg (ok_button, GTK_STATE_NORMAL, &colorGreen);
      ...
      if (jcset->isValid())
        {
          ...
          lab << "IR file info: " << format << " BIT | " << sr << " HZ "<< endl;
          lab << chans            << " Channel(s) "
          << framecount       << " Samples ";
      ...
      gdk_color_parse("#a6a9aa", &colorGreen);
      gtk_widget_modify_fg (gx_gui::label6, GTK_STATE_NORMAL, &colorGreen);
      ...
      if (status == kNoError)
        {
          ...
          return;
        } else if (status == kErrorResample){
            remove(resampled.c_str());
        }
          ...
          if (jcset->isValid())
            {
              ...
              /* A no resampled file could be used
              if the user wish so.*/
